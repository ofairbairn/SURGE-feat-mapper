"""Prepare Fashion-MNIST from .npy files and run SURGE VAE workflow.

This script:
1) Loads the four Fashion-MNIST arrays from disk.
2) Flattens images to tabular pixel columns.
3) Builds a single CSV dataset expected by examples/configs/fashion_mnist_vae.yaml.
4) Runs the SURGE workflow using the registered model key "pytorch.vae".
"""
...
import surge  # noqa: F401 - triggers model adapter registration
...
SPEC_PATH = Path("examples/configs/fashion_mnist_vae.yaml")
